...
export class AuthService {
  ...
  async signupLocal(dto: CreateUserDto): Promise<boolean> {
  ...
  async signinLocal(dto: AuthDto): Promise<LoginResponseType> {
    const user = await this.userService.getUserByEmail(dto.email);
    if (!user) AccessDeniedError();

    const password_matches = await bcrypt.compare(dto.password, user.password);
    if (!password_matches) AccessDeniedError();

    const tokens = await this.getTokens(user);
    await this.updateRtHash(user, tokens.refresh_token);
    return { ...tokens, active: user.active };
  }
